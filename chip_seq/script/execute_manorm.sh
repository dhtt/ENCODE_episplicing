...
pair_chunks=$INPUT_PATH/pair_chunks
mkdir $pair_chunks
split -l 11 --numeric-suffixes $INPUT_PATH/all_pairs.txt $pair_chunks/chunk_
...
execute_manorm_parallel() {
    ...
    if (ls $INPUT_PATH/manorm_result | grep "$epi1"_"$epi2"); then
        (
