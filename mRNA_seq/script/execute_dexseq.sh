...
execute_dexseq_parallel() {
    ...
    if (ls $f/count | grep "$epi1" ) && (ls $f/count | grep "$epi2" )
    then (
        ...
        then (
            ...
            Rscript $ENCODE_Escript/DEXSeq_analysis.R -a "$epi1" -b "$epi2" -f  $f/count -g $ENCODE_REFGEN/reference_genome.gtf -n 8
            echo "DEXSeq-ing pair "$epi1"_"$epi2"" >> execute_dexseq.log
        )
        fi
        )
    else echo "Pair "$epi1"_"$epi2" does not exist"
    fi
}
